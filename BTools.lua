...
function main()
    ...
    sampRegisterChatCommand('updinfo', function() sampAddChatMessage('да это новая версия', -1) end)
	...
	if pns.v then
        ...
        if imgui.BeginPopupModal(u8"MUTE IC", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
            ...
            if imgui.AnimatedButton(u8'NONRP GAME', bsize) then
                ...
                snotf.addNotification(string.format('Вы выдали IC мут игроку '..plname..'['..plID..'].\nПричина: НРП.'), 4, 1)
                imgui.CloseCurrentPopup()
            end
